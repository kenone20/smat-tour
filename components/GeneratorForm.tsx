...
import { INTEREST_OPTIONS, INITIAL_FORM_STATE } from '../constants';
import { useTranslation } from '../contexts/LanguageContext';


interface GeneratorFormProps {
  onGenerate: (formData: GeneratorFormState, type: GenerationType) => void;
  isLoading: boolean;
}
...
const GeneratorForm: React.FC<GeneratorFormProps> = ({ onGenerate, isLoading }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onGenerate(formData, 'full');
  };

  return (
    <form onSubmit={handleSubmit} className="space-y-8 p-8 bg-white rounded-2xl shadow-lg">
      <div className="text-center">
        <h2 className="text-3xl font-bold text-secondary">{t('form_title')}</h2>
        <p className="text-slate-500 mt-2">{t('form_subtitle')}</p>
      </div>
      ...
      <div>
        <label className="block text-sm font-medium text-slate-700 mb-2">{t('form_interests')}</label>
      ...
      <div>
        <label className="block text-sm font-medium text-slate-700 mb-2">{t('form_pace')}</label>
        <div className="grid grid-cols-3 gap-2 rounded-lg bg-slate-100 p-1">
      ...
      <div className="space-y-3">
        <button 
          type="submit" 
          disabled={isLoading}
          className="w-full bg-primary hover:bg-primary-focus text-white font-bold py-4 px-4 rounded-lg transition-all duration-300 ease-in-out disabled:bg-slate-400 disabled:cursor-not-allowed flex items-center justify-center text-lg"
        >
          {isLoading ? (
            <>
        ...
        <button
          type="button"
          onClick={() => onGenerate(formData, 'quick')}
          disabled={isLoading}
          className="w-full bg-white border border-primary text-primary hover:bg-primary/10 font-bold py-3 px-4 rounded-lg transition-all duration-300 ease-in-out disabled:bg-slate-200 disabled:text-slate-500 disabled:border-slate-200 disabled:cursor-not-allowed flex items-center justify-center"
        >
...
const BudgetOption: React.FC<BudgetOptionProps> = ({ label, value, current, setter }) => (
    <button type="button" onClick={() => setter(p => ({...p, budget: value}))} className={`px-3 py-2 text-sm font-medium rounded-md transition-colors ${current === value ? 'bg-primary text-white shadow' : 'text-slate-600 hover:bg-slate-200'}`}>
        {label}
    </button>
);
...
const PaceOption: React.FC<PaceOptionProps> = ({ label, value, current, setter }) => (
    <button type="button" onClick={() => setter(p => ({...p, pace: value}))} className={`px-3 py-2 text-sm font-medium rounded-md transition-colors ${current === value ? 'bg-primary text-white shadow' : 'text-slate-600 hover:bg-slate-200'}`}>
        {label}
    </button>
);
